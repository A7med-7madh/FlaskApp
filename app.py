...
def gconnect():
    ...
    flash("Hi %s, now you are logging in ..." % login_session['username'])
    print("Done!")
    return output
...
def logout():
    ...
    if 'username' in login_session:
        ...
        flash("Sorry, You were not logged in, please log in again.")
        return redirect(url_for('home'))
...
def add_category():
    ...
    if request.method == 'POST':
        if request.form['new-category-name'] == '':
            flash('Sorry, This Field cannot be Empty, you have to fill it...')
            return redirect(url_for('home'))

        category = session.query(Category).\
            filter_by(name=request.form['new-category-name']).first()
        if category is not None:
            flash('The Entered Category Already Exists, Please Enter a new one.')
        ...
        session.commit()
        flash('The New Category " %s "  has been Successfully Created.' % new_category.name)
        return redirect(url_for('home'))
    else:
...
def add_item():
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash('A New Item has been Successfully Created!')
        return redirect(url_for('home'))
    else:
...
def view_item(item_id):
    ...
    if exists_item(item_id):
        ...
        flash('Sorry, We are unable to process your request right now. please, try again...')
        return redirect(url_for('home'))
...
def edit_item(item_id):
    ...
    if not exists_item(item_id):
        flash("Sorry, We are unable to process your request right now.")
        return redirect(url_for('home'))

    item = session.query(Item).filter_by(id=item_id).first()
    if login_session['user_id'] != item.user_id:
        flash("Sorry, You are not authorised to Access that Page.")
        return redirect(url_for('home'))
